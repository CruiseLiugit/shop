...
<head>
	...
	// 获取当前用户当前菜单拥有的按钮
	$.ajax({
	   url:'${contextPath }/menus/getUserToolbar.do?menusCode=f83f35f8-2716-31c2-83f2-4cd66ad75ehd', 
	   type:'post',	   
    ...
    grid = $('#grid').datagrid({
        title: '系统角色列表',
        iconCls: 'icon-save',
        url: '${contextPath}/role/queryAllRoleList.do',
        sortName: 'ID',
        sortOrder: 'desc',
        idField: 'roleCode',
        pageSize:10,
        ...
        columns: [[        			
                    ...
                    { field: 'description', title: '角色描述', width: 150, sortable: true }
                ]],
        pagination: true,
        rownumbers: true,
        singleSelect: false,
    ...
    if (num == 0) {
        ...
        return;
   		...
   		var roleEdit= $('#edit-window').dialog({  // 打开一个编辑对话框
	        href:"${contextPath }/role/getRoleByCode.do?id="+ rows[0].id, // 对话框引用的地址
	        title: '角色详情',
	        width: 600,
	        modal: true,
...
function add() {
    win.window('open');
    var roleInput= $('#edit-window').dialog({
	        href:"${contextPath }/jsp/role/role-input.jsp",
	        title: '新增系统角色',
	        width: 600,
	        modal: true,
	        maximizable:true,
...
function edit() {
    ...
    if (num == 0) {
        ...
        $.messager.alert('提示', '您选择了多条记录,只能选择一条记录进行修改!', 'info');
        return;
    }else if(rows[0].id=="d22f30b8-2716-41d2-84f2-4cd56bb75ecc"){ // 系统管理员的角色id
       $.messager.alert('提示', '系统管理员角色禁止修改!', 'info');
    }else{
   		var roleEdit= $('#edit-window').dialog({  // 打开一个编辑对话框
	        href:"${contextPath }/role/getRoleById.do?id="+ rows[0].id, // 对话框引用的地址
	        title: '修改系统角色',
	        width: 600,
	        modal: true,
	        maximizable:true, // 是否可以最大化
...
function del() {
    ...
    for(var i=0;i<arr.length;i++){
        if(arr[i]=="d22f30b8-2716-41d2-84f2-4cd56bb75ecc"){
        flag = false;
        }
    } 
    if(flag){
        ...
        $.messager.confirm('提示信息', '您确认要删除吗?', function (data) {
            if (data) {
                $.ajax({
                    url: '${contextPath }/role/deleteRoles.do?ids=' + arr2str(arr),
                    type: 'GET',
                    timeout: 1000,
     ...
     $.messager.alert('提示', '系统管理员角色禁止删除!', 'info');
     return;
    }
...
// 查询所有角色列表
function showAll() {
    grid.datagrid({ url: '${contextPath}/role/queryAllRoleList.do' });
...
function SearchOK() {
    ...
    grid.datagrid({ url: '${contextPath }/role/getRoleByRoleName.do',pageNumber:1, queryParams: {roleName: s_title} });
    $('#grid').datagrid("addToolbarItem",toolbar);
}
