...
$(function () {
    ...
    var url='${contextPath }/propertyValue/queryCategoryProperty.do';;
    grid = $("#grid").treegrid({
    	title: '类目属性列表',	
		iconCls: 'icon-edit',
		toolbar: toolbar,
        ...
        columns:[[
		          {field:'showName',title:'类目/属性展示名称',width:300},
		          {field:'name',title:'类目/属性名称',width:200},
		          {field:'type',title:'类目/属性类型',width:120},
		          {field:'propertyValue',title:'属性值',width:500}
		        ]]
	});
...
function add() {
    ...
    if (num == 0) {
    	...
    	if(rows[0].node!='category'){
    		var editCategory= $('#edit-window').dialog({
   		        href:"${contextPath }/propertyValue/getValuesByCategoryProperty.do?categoryPropertyCode="+ rows[0].id+"&option=add",
   		        title: '修改类目',
   		        width: 500,
   		        modal: true,
   		        maximizable:true,
    		...
    		$.messager.alert('警告', '只能操作类目下属性节点!', 'warning');
    	}
    }
}
...
function edit() {
    ...
    if (num == 0) {
    	...
    	if(rows[0].node!='category'){
    		var editCategory= $('#edit-window').dialog({
   		        href:"${contextPath }/propertyValue/getValuesByCategoryProperty.do?categoryPropertyCode="+ rows[0].id+"&option=edit",
   		        title: '修改类目',
   		        width: 500,
   		        modal: true,
   		        maximizable:true,
...
function view(){
    ...
    if (num == 0) {
    	...
    	if(rows[0].node!='category'){
    		var editCategory= $('#edit-window').dialog({
   		        href:"${contextPath }/propertyValue/getValuesByCategoryProperty.do?categoryPropertyCode="+ rows[0].id+"&option=select",
   		        title: '修改类目',
   		        width: 500,
   		        modal: true,
   		        maximizable:true,
