...
$(function () {
	...
	$.ajax({ 
		url:'${contextPath }/menus/getUserToolbar.do?menusCode=a387a429-a750-4b50-8b56-e8b2d14d3c9a', // 获取当前用户当前菜单拥有的按钮 
		type:'post', 
    ...
    var url;
	if($("#select").val()=='null'||$("#select").val()=='0'){//默认和选择全部时正常整体显示
		url='${contextPath }/category/list.do';
	}else{
		url='${contextPath }/category/list.do?select='+$("#select").val();
	}
    grid = $("#grid").treegrid({
    	title: '类目列表',	
		iconCls: 'icon-edit',
    ...
    if (num == 0) {
        ...
        return;
   		...
   		var editCategory= $('#edit-window').dialog({
	        href:"${contextPath }/category/getCategoryByCode.do?categoryCode="+ rows[0].id+"&option=edit",
	        title: '修改类目',
	        width: 500,
	        modal: true,
	        maximizable:true,
    	 ...
    	 $.messager.confirm('提示信息', '您确认要删除吗?', function (msg) {
             if (msg) {
            	 if(rows[0].state=='open'){
            		 $.get('${contextPath}/category/delete.do',{categoryCode:rows[0].id},function(data){
							if(data=='0'){
								$.messager.alert('警告', '只能删除叶子节点!', 'warning');
							}else{
								$("#grid").treegrid('remove',rows[0].id);
...
function view(){
    ...
    if (num == 0) {
        ...
        return;
   		...
   		var editCategory= $('#edit-window').dialog({
	        href:"${contextPath }/category/getCategoryById.do?categoryCode="+ rows[0].id,
	        title: '修改类目',
	        width: 500,
	        modal: true,
	        maximizable:true,
