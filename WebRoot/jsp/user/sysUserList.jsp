...
$(function () {
	...
	$.ajax({ 
		url:'${contextPath }/menus/getUserToolbar.do?menusCode=a23f35f8-2716-31c2-83f2-4cd66ad75ehd', // 获取当前用户当前菜单拥有的按钮 
		type:'post', 
    ...
    grid = $('#grid').datagrid({
        title: '系统用户列表',
        iconCls: 'icon-save',
        url: '${contextPath }/sysUserManager/getAllSysUserList.do',
        sortName: 'ID',
        sortOrder: 'desc',
        idField: 'id',
        pageSize:10,
        ...
        columns: [[
                    ...
                    { field: 'userPhone', title: '联系方式', width: 150, sortable: true },
...
//判断用户选中的删除或者修改对象是否包含系统管理员
function getSelectedArrContainsAdmin(){
	var rows = grid.datagrid('getSelections');
	var adminId;
	var flag = false;
	for( var i=0;i<rows.length;i++){
		...
		if(adminId.match('550E8400-E29B-11D4-A716-446655440000')){//系统管理员Id
			flag = true;
		}
	}
	return flag;
...
function add() {
    var editSysUser= $('#edit-window').dialog({
	        href:"${contextPath }/jsp/user/editSysUser.jsp",
	        title: '新增系统用户',
	        width: 350,
	        modal: true,
	        maximizable:true,
...
function edit() {
    ...
    if (num == 0) {
    	...
    	var b =getSelectedArrContainsAdmin();
    	if(b){//选中了修改系统管理员
    		$.messager.alert('警告', '不能修改系统管理员。', 'warning');
    		return;
    	}
   		var editSysUser= $('#edit-window').dialog({
	        href:"${contextPath }/sysUserManager/getSysUserById.do?id="+ rows[0].id,
	        title: '修改系统用户',
	        width: 500,
	        modal: true,
	        maximizable:true,
...
function del() {
    ...
    if (arr.length>0) {
    	if(b){//选中了删除系统管理员
    		$.messager.alert('警告', '不能删除系统管理员。', 'warning');
    		return;
    	}
        $.messager.confirm('提示信息', '您确认要删除吗?', function (data) {
            if (data) {
                $.ajax({
                    url: '${contextPath }/sysUserManager/deleteSysUser.do?userCode=' + arr2str(arr),
                    error: function () {
    ...
    if (num == 0) {
        ...
        return;
   		...
   		var viewPave= $('#edit-window').dialog({
	        href:"${contextPath }/sysUserManager/getSysUserByUserCode.do?userCode="+ rows[0].id,
	        title: '查看铺设点详情',
	        width: 750,
	        modal: true,
	        closed: true,
...
function showAll() {
    grid.datagrid({ url: '${contextPath }/sysUserManager/getAllSysUserList.do' });
    $('#grid').datagrid("addToolbarItem",toolbar); // 为toolbar 赋值
}
...
function SearchOK() {
    ...
    grid.datagrid({ url: '${contextPath }/sysUserManager/getSysUserByLoginName.do',
    				pageNumber:1, 
